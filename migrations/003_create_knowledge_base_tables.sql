...
-- 2.1 identity_provider - Identity providers
CREATE TABLE IF NOT EXISTS identity_provider (
    ...
    CONSTRAINT uq_identity_provider_slug UNIQUE (slug)
);

-- Seed identity providers
INSERT INTO identity_provider (name, slug, display_name, status) VALUES
    ('google_workspace', 'google-workspace', 'Google Workspace', 'active'),
    ('microsoft_365', 'microsoft-365', 'Microsoft 365', 'coming_soon')
ON CONFLICT (slug) DO NOTHING;

-- 2.2 app_category - App categories
CREATE TABLE IF NOT EXISTS app_category (
   id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    created_at              TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    updated_at              TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_app_category_slug UNIQUE (slug)
);

-- Seed app categories
INSERT INTO app_category (name, slug, display_name) VALUES
...
CREATE TABLE IF NOT EXISTS product (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    identity_provider_id    INTEGER REFERENCES identity_provider(id),
    app_category_id         INTEGER REFERENCES app_category(id),
...
CREATE INDEX IF NOT EXISTS idx_product_identity_provider ON product(identity_provider_id);
CREATE INDEX IF NOT EXISTS idx_product_app_category ON product(app_category_id);
...
CREATE INDEX IF NOT EXISTS idx_product_auth_config_identity_provider ON product_auth_config(identity_provider_id);
