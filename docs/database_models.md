...
## 4.1.1 workspace_group

Teams/groups discovered from org's Google Workspace (via Directory API).
...
CREATE TABLE workspace_group (
    ...
    provider_group_id       VARCHAR(255) NOT NULL,     -- Google: group.id
...
CREATE INDEX idx_workspace_group_org ON workspace_group(organization_id);
CREATE INDEX idx_workspace_group_email ON workspace_group(email);
```

## 4.1.2 group_membership

Links workspace users to workspace groups.
...
CREATE TABLE group_membership (
    ...
    workspace_group_id      INTEGER NOT NULL REFERENCES workspace_group(id) ON DELETE CASCADE,
...
CREATE TABLE discovered_app (
   id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
    ...
    metadata                JSONB -- Entire raw object from google
...
CREATE TABLE app_authorization (
   id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
    ...
    CONSTRAINT uq_app_authorization_app_user UNIQUE(discovered_app_id, workspace_user_id)
);
